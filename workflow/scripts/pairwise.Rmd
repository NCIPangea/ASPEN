...
# color by sample
h <- c()
for (i in rownames(m.pca$rotation)) {
  h <- c(h, e$sample[e$replicate == i])
}
```
\newline
...
if (ncol(m.pca$rotation) == 2) {
  ...
  p <- plot_ly(as.data.frame(m.pca$x[, 1:3]),
    x = ~PC1, y = ~PC2, z = ~PC3, hoverinfo = "text", color = h,
    hovertext = rownames(toplot)
  ) %>%
    add_markers() %>%
    layout(
      title = "Jaccard score 3-d PCA plot",
      scene = list(
        xaxis = list(title = paste0("PC1 (", m.pc1, "%)")),
...
p <- plot_ly(as.data.frame(m.pca$x[, 1:2]),
  x = ~PC1, y = ~PC2, hoverinfo = "text", color = h,
  hovertext = rownames(toplot)
) %>%
  add_markers() %>%
